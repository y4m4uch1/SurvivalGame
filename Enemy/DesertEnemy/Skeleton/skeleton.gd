...
var cone_attack_visual: Polygon2D
...
# Attack variables
var cone_attack_angle: float = deg_to_rad(180.0)
...
func _ready():
	...
	# Initialize cone attack Polygon2D
	cone_attack_visual = Polygon2D.new()
	cone_attack_visual.color = Color(1, 0, 0, 0.3)
	cone_attack_visual.visible = false
	add_child(cone_attack_visual)
	draw_cone_points(cone_attack_visual, attack_range, cone_attack_angle)
	...
	# Initialize detection area collision shape
...
func update_move_animation(delta: float):
	...
	if velocity.x < -10:
		sprite.flip_h = true
		collision_shape.position = Vector2(-2, 1)
	elif velocity.x > 10:
		sprite.flip_h = false
		collision_shape.position = Vector2(3, 1)
...
func attack():
	...
	var frame_duration = 1.0 / sprite.sprite_frames.get_animation_speed("attack")
	...
	draw_cone_points(cone_attack_visual, attack_range, cone_attack_angle, angle_to_player)
	cone_attack_visual.visible = true
	cone_attack_visual.color = Color(1, 0, 0, 0.3)
	
	var tween = create_tween()
	tween.tween_interval(warning_duration)
	tween.tween_callback(func():
		cone_attack_visual.color = Color(1, 0, 0, 0.8)
...
func draw_cone_points(polygon: Polygon2D, range: float, angle: float, rotation: float = 0.0):
	var points = create_cone_points(range, angle, rotation)
	polygon.polygon = points

func create_cone_points(range: float, angle: float, rotation: float) -> PackedVector2Array:
	var points = []
	var num_points = 16
	...
	for i in range(num_points + 1):
		...
		var current_angle = rotation - half_angle + t * angle
