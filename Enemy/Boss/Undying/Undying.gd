...
@export var shockwave_range: float = 100.0
...
@onready var theme_sound = $Theme  # Reference to theme sound player
...
func _ready():
	...
	if theme_sound and theme_sound.stream:
		theme_sound.stream.loop_mode = AudioStreamWAV.LOOP_FORWARD  # Enable looping
		theme_sound.play()
...
func update_move_animation(delta: float):
	...
	if move_animation_timer >= move_animation_speed:
		current_move_frame = (current_move_frame + 1) % 5
...
func update_attack_animation(delta: float):
	...
	if attack_animation_timer >= attack_animation_speed:
		if is_attacking and sprite.frame >= 6 and sprite.frame <= 10: # Rectangle attack
			current_attack_frame = (current_attack_frame + 1) % 5
			sprite.frame = 6 + current_attack_frame
		elif is_attacking and sprite.frame >= 11 and sprite.frame <= 18: # Shockwave attack
			current_attack_frame = (current_attack_frame + 1) % 8
...
func attack_rectangle():
	...
	# Create rectangular attack visualization
	var rectangle = Polygon2D.new()
	rectangle.color = Color(1, 0, 0, 0.3)
	rectangle.visible = true
	add_child(rectangle)
	
	# Define rectangle parameters
	var rect_width = 50.0  # Width of the rectangle
	var rect_length = attack_range * 2  # Length extends to twice attack range
	var direction = (player.global_position - global_position).normalized()
	var angle = atan2(direction.y, direction.x)
	
	# Generate rectangle points (positioned in front of the boss)
	var points = []
	var half_width = rect_width / 2
	var forward = Vector2(cos(angle), sin(angle)) * rect_length
	var side = Vector2(-sin(angle), cos(angle)) * half_width
	points.append(Vector2.ZERO - side)  # Bottom-left (at boss position)
	points.append(Vector2.ZERO + side)  # Bottom-right (at boss position)
	points.append(forward + side)  # Top-right
	points.append(forward - side)  # Top-left
	...
	tween.tween_callback(func():
		rectangle.color = Color(1, 0, 0, 0.8)
		var space_state = get_world_2d().direct_space_state
		var query = PhysicsShapeQueryParameters2D.new()
		var rect_shape = ConvexPolygonShape2D.new()
		var shape_points = []
		shape_points.append(Vector2.ZERO - side)
		shape_points.append(Vector2.ZERO + side)
		shape_points.append(forward + side)
		shape_points.append(forward - side)
		rect_shape.points = shape_points
		...
		for result in results:
			if result.collider == player and player.has_method("take_damage"):
				...
				print("Undefeated attacked player with rectangle for " + str(attack_damage) + " damage!")
				break
	)
...
func attack_shockwave():
	...
	# Create circular shockwave visualization
	var shockwave = Polygon2D.new()
	shockwave.color = Color(1, 0.5, 0, 0.3)
	shockwave.visible = true
	add_child(shockwave)
	
	# Generate circle points for shockwave
	var points = []
	var num_points = 32
	for i in range(num_points + 1):
		var angle = i * 2 * PI / num_points
		var point = Vector2(cos(angle), sin(angle)) * shockwave_range
		points.append(point)
	shockwave.polygon = points
	
	# Create tween for shockwave effect
	var tween = create_tween()
	tween.tween_interval(0.8)
	tween.tween_callback(func():
		shockwave.color = Color(1, 0.5, 0, 0.8)
		# Check if player is within shockwave range
		if global_position.distance_to(player.global_position) <= shockwave_range:
			if player.has_method("take_damage"):
				player.take_damage(attack_damage)
				print("Undefeated attacked player with shockwave for " + str(attack_damage) + " damage!")
	)
	tween.tween_interval(0.3)
	tween.tween_callback(func():
