...
func _ready() -> void:
	...
	if world:
		world.connect("tilemap_generated", Callable(self, "spawn_totem"))
		# Kết nối sự kiện triệu hồi boss nếu có
		if world.has_signal("boss_summoned"):
			world.connect("boss_summoned", Callable(self, "_on_boss_summoned"))
...
func _on_boss_summoned() -> void:
	# Khi boss được triệu hồi, spawn totem ở vị trí mới
	spawn_totem()
