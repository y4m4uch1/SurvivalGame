...
func break_tin():
	...
	if ground_node:
		for i in range(3):  # Tạo 3 mảnh đá
			var tin_instance = create_tin_item()
			tin_instance.position = global_position + Vector2(randi_range(-10, 10), randi_range(-10, 10))
			ground_node.add_child(tin_instance)
...
func create_tin_item() -> Node2D:
	var item_scene = preload("res://GUI/Inventory/Item.tscn")
	var tin = item_scene.instantiate()
