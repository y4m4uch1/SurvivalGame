...
func break_mud():
	...
	if ground_node:
		for i in range(3):  
			var mud_instance = create_mud_item()
			mud_instance.position = global_position + Vector2(randi_range(-10, 10), randi_range(-10, 10))
			ground_node.add_child(mud_instance)
			
	
	# Tìm spawner và xóa entity (mud pile) khỏi danh sách
...
func create_mud_item() -> Node2D:
	var item_scene = preload("res://GUI/Inventory/Item.tscn")
	var mud = item_scene.instantiate()
