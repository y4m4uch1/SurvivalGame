...
var required_tool: String = "PickAxe"  
var required_tier: int = 5
...
func break_diamond():
	...
	if ground_node:
		for i in range(2):  
			var diamond_instance = create_diamond_item()
			diamond_instance.position = global_position + Vector2(randi_range(-10, 10), randi_range(-10, 10))
			ground_node.add_child(diamond_instance)
			
	
	# Tìm spawner và xóa entity (diamond) khỏi danh sách
...
func create_diamond_item() -> Node2D:
	var item_scene = preload("res://GUI/Inventory/Item.tscn")
	var diamond = item_scene.instantiate()
	
	# Gán thuộc tính cho item
	diamond.item_name = "Diamond"
	return diamond
