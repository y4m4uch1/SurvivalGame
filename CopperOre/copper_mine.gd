...
func break_copper():
	...
	if ground_node:
		for i in range(3):  # Tạo 3 mảnh đá
			var copper_instance = create_copper_item()
			copper_instance.position = global_position + Vector2(randi_range(-10, 10), randi_range(-10, 10))
			ground_node.add_child(copper_instance)
...
func create_copper_item() -> Node2D:
	var item_scene = preload("res://GUI/Inventory/Item.tscn")
	var copper = item_scene.instantiate()
